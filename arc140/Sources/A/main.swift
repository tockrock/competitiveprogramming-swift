...
func main() {
    ...
    for i in 1...N {
        ...
        for j in stride(from: 0, to: N, by: i) {
            let rep = Array(S[j..<j+i])
            count[rep, default: 0] += 1
        }
        
        let sorted = count.sorted { (lhs, rhs) in
            lhs.value > rhs.value
        }
                
        next: for l in 0..<sorted.count {
            var totalDiff = 0
            let base = sorted[l].key
            for m in 0..<sorted.count where l != m {
                let compare = sorted[m].key
                var localDiff = 0
                for n in 0..<base.count {
                    if base[n] != compare[n] {
                        localDiff += 1
                    }
                }
                totalDiff += localDiff * sorted[m].value
                
                
                guard totalDiff <= K else { continue next }
            }
            print(i)
            return
        }
