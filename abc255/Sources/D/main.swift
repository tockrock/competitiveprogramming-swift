...
func main() {
    ...
    var adjustFromSmaller = [0]
    
    for i in 1..<n {
        adjustFromSmaller.append((As[i] - As[i-1]) * i + adjustFromSmaller[i - 1])
    }

    var adjustFromLarger = [0]
    for i in 1..<n {
        adjustFromLarger.append((As[n - i] - As[n-i-1]) * i + adjustFromLarger[i-1])
    }
    
    adjustFromLarger.reverse()
    ...
    for _ in 0..<q {
        let x = readInt()
        let index = binary(x: x)
        
        if x < As[index] {
            print((As[index] - x) * n + adjustFromLarger[index])
            continue
        }
        
        if index == n - 1 {
            print((x - As[index]) * n + adjustFromSmaller[index])
            continue
        }
                    
        if x == As[index + 1] {
            print(adjustFromSmaller[index+1] + adjustFromLarger[index + 1])
            continue
        }
        
        
        var total = (x - As[index]) * (index + 1) + adjustFromSmaller[index]
        total += (As[index+1] - x) * (n - index - 1) + adjustFromLarger[index + 1]
