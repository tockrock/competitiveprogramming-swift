...
func main() {
    ...
    for _ in 0..<n {
        position.append(readInts().tupled())
    }
    
    var longestOverall = Double(Int.min)
    for i in 0..<n {
        var shortest = Double(Int.max)
        let from = position[i]
        for a in As {
            let holder = position[a-1]
            let x = Double(from.x - holder.x)
            let y = Double(from.y - holder.y)
            let distance = sqrt(pow(x, 2) + pow(y, 2))
            shortest = min(shortest, distance)
        }
        longestOverall = max(longestOverall, shortest)
    }
