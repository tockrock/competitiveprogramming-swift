...
func run(readLine: () -> String?, print: (Any...) -> Void) {
    ...
    let digits = String(A.max()!, radix: 2).count
    myDebugPrint(digits)
    ...
    for a in A {
        var binary = String(a, radix: 2)
        binary = String(repeating: "0", count: digits - binary.count) + binary
        graph.append(binary.map {Int(String($0))!})
    }
