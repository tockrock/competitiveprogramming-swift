...
func main() {
    ...
    var nameUnique = [String: Bool]()
    var people = [Set<String>]()
    
    for _ in 0..<N {
        let names = Set(readStrings())
        people.append(names)
        for name in names {
            guard nameUnique[name] == nil else {
                nameUnique[name] = false
                continue
            }
            nameUnique[name] = true
        }
    }
    
    var ans = true
    for names in people {
        if names.filter({nameUnique[$0]!}).isEmpty {
            ans = false
            break
        }
    }
