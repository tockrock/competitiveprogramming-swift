...
func main() {
    ...
    let (_, X, Y) = readInts().tupled()
    let A = readInts()
    
    func sliceArray(_ original: [Int]) -> [[Int]] {
        var arrays = [[Int]]()
        var l = 0
        for r in 0..<original.count {
            let numb = original[r]
            
            guard numb < Y || X < numb else { continue }
            
            arrays.append(Array(original[l..<r]))
            l = r + 1
        }
        
        if l < original.count - 1 {
            arrays.append(Array(original[l...]))
        }
        return arrays
    }
    
    func countArray(_ array: [Int]) -> Int {
        var hasMax = false
        var hasMin = false
        var maxIndex = 0
        var minIndex = 0
        ...
        for i in array.startIndex..<array.endIndex {
            ...
            guard hasMin && hasMax else { continue }
            
            let l = min(minIndex, maxIndex)
            let r = max(minIndex, maxIndex)
            
            
            count += (l+1) * (array.count - r)
            if l < array.count - 1 {
                let nextArray = Array(array[(l+1)...])
                count += countArray(nextArray)
            }
            break
        }
        return count
    ...
    let arrays = sliceArray(A)
    var ans = 0
    for array in arrays {
        ans += countArray(array)
    }
    print(ans)
