...
func main() {
    ...
    for _ in 0..<Q {
        ...
        case .add:
            queue.append(Balls(x: input[1], c: input[2]))
        case .count:
            var count = 0
            var remaining = input[1]
            while remaining > 0 {
                guard queue[index].x > 0 else {
                    index += 1
                    continue
                }
                
                let usable = min(remaining, queue[index].c)
                count += usable * queue[index].x
                remaining -= usable
                
                if queue[index].c == usable {
                    index += 1
                } else {
                    queue[index].c -= usable
                }
            }
            print(count)
        }
