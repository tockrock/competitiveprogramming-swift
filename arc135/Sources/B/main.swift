...
func main() {
    ...
    for s in S {
        let count = x.count
        x.append(s - x[count-1] - x[count-2])
    }
    
    var c1 = Int.max, c2 = Int.max, c3 = Int.max
    
    for i in stride(from: 0, to: n+2, by: 3) {
        c1 = min(c1, x[i])
    }
    for i in stride(from: 1, to: n+2, by: 3) {
        c2 = min(c2, x[i])
    }
    for i in stride(from: 2, to: n+2, by: 3) {
        c3 = min(c3, x[i])
    }
    c1 *= -1
    c2 *= -1
    ...
    print(A.joinedAsString(separator: " "))
