...
func main() {
    ...
    var point = [(x: Int, y: Int, P: Int)]()
    
    for _ in 0..<n {
        let (x, y, P) = readInts().tupled()
        point.append((x, y, P))
    }
    ...
    for i in 0..<n {
        for j in 0..<n {
            let distance = abs(point[i].x - point[j].x) + abs(point[i].y - point[j].y)
            graph[i].append(Int(ceil((Double(distance) / Double(point[i].P)))))
        }
    }
    ...
    for i in 0..<n {
        travelled = [Bool](repeating: false, count: n)
        var longest = 0
        var pq = PriorityQueue<(destination: Int, distance: Int)>([(i, 0)], order: {$0.distance < $1.distance})
        
        while let next = pq.pop() {
            guard !travelled[next.destination] else { continue }
            travelled[next.destination] = true
            longest = max(longest, next.distance)
            for j in 0..<n where !travelled[j] {
                pq.push((j, graph[next.destination][j]))
            }
        }
