...
func main() {
    ...
    let (n, _, _) = readInts().tupled()
    let As = readInts()
    let Ls = readInts()
    
    var box = [Bool](repeating: false, count: n+1)
    for a in As {
        box[a] = true
    }
    ...
    for l in Ls {
        ...
        box.swapAt(index, index+1)
    }
    print(box.enumerated().filter({$0.element}).map({$0.offset}).joinedAsString(separator: " "))
