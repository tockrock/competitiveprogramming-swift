...
func main() {
    ...
    let (N, A, B) = readInts().tupled()
    
    var startColorWhite = true

    for _ in 0..<N {
        for _ in 0..<A {
            var cellColorWhite = startColorWhite
            for _ in 0..<N {
                for _ in 0..<B {
                    print(cellColorWhite ? "." : "#", terminator: "")
                }
                cellColorWhite.toggle()
            }
            print()
        }
        startColorWhite.toggle()
    }
