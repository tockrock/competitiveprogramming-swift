...
func main() {
    ...
    var primeChecker = [Bool](repeating: true, count: maxQ + 2)
    primeChecker[0] = false
    primeChecker[1] = false
    
    var primes = [Int]()
    
    for i in 2...maxQ where primeChecker[i] {
        primes.append(i)
        for multi in stride(from: i * 2, through: maxQ, by: i) {
            primeChecker[multi] = false
        }
    }

    let N = readInt()
    var ans = 0
    nextP: for i in 0..<(primes.count - 1) {
        let p = primes[i]
        guard p * p * p * p < N else { break }
        for q in primes[(i+1)...] {
            guard p * q * q * q <= N else { continue nextP }
